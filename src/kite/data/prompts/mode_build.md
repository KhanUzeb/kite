...
4. Use `bash` for tests, git status, and builds — never for file edits you could do with `edit`. Do not `git commit` or `git push` unless they asked.
...
Stay inside the workspace. Prefer reversible edits. Never `git push`. If they want a commit, they will say so or run `/commit`.
